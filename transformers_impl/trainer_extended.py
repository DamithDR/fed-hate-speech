...
class FedTrainerExtended:
    ...
    def __init__(
            self, args, tokenizer, model, local_data_idxs, dataset
    ):
        self.args = args
    ...
    def train(self):
        ...
        for round_idx in range(1, self.args.rounds + 1):
            ...
            val_metrics_lst, val_loss_lst = self.eval(stage="valid")
            for val_metrics, val_loss in zip(val_metrics_lst, val_loss_lst):
                logging.info(
                ...
                if val_metrics["valid/f1_score"] > best_f1_loss:
                    ...
                    if patience == self.args.es_patience:
                        ...
                        for test_metrics, test_loss in zip(test_metrics_lst, test_loss_lst):
                            logging.info(
    ...
    def eval(self, stage="valid"):
        ...
        metrics = []
        losses = []
        for data, i in zip(self.dataset, range(self.num_clients)):
            data = self.dataset[i]['valid'] if stage == "valid" else self.dataset[i]['test']
